        ...
        try:
            ...
            try:
                ...
                raise ScrapingError(
                    ...
                    instaloader.exceptions.PostChangedException) as e:
                logger.error(f"Instagram post {shortcode} is unavailable: {type(e).__name__}: {e}")
                raise ScrapingError(
                    "This Instagram post is no longer available (may have been deleted, made private, or temporarily inaccessible).",
                    self.platform_name, url
                )
